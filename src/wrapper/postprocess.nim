...
proc createTypesFile*(pattern:string) =
  ...
  for d in dirs:
    ...
    if tmp[tmp.high] == "":
      tmp = tmp[tmp.low ..< tmp.high]
    var prefix = tmp[tmp.high]

  #for fname in walkFiles("./tk*/*/*.nim"): 

    # Get the files within the subfolder
    for fname in walkFiles(fmt"{d}/*.nim"):
      if not (fname in @[fmt"./{d}/gen.nim", fmt"./{d}/ed.nim"]):
...
#createTypesFile("./fitz/")


#reorderContent("./fitz/fitz_types.nim")
